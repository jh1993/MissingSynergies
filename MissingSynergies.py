...
class SecretsOfBlood(Upgrade):
    ...
    def get_description(self):
        return ("Each turn, each stack of bloodrage on each ally has a 50% chance to have its remaining duration increased by [1_turn:duration]. Bloodrage is typically generated by [demon] minions with \"blood\" in their names.\n"
                "Whenever an ally deals damage to an enemy with an attack, each bloodrage stack on the attacker inflicts a stack of bleed on the target for the same duration, which deals [physical] damage per turn equal to the bloodrage stack's damage bonus and then has a 50% chance to be removed.\n"
    ...
    def on_damaged(self, evt):
        if not are_hostile(evt.unit, self.owner) or not evt.source or not evt.source.owner or are_hostile(evt.source.owner, self.owner) or not isinstance(evt.source, Spell):
            return
