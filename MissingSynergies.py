...
class EnfleshedBuff(Buff):
    ...
    def on_unapplied(self):
        alive = self.owner.is_alive()
        self.owner.cur_hp -= self.hp
        self.owner.cur_hp = max(1 if alive else 0, self.owner.cur_hp)
...
class MalignantGrowthSpell(Spell):
    ...
    def get_description(self):
        return ("Enemies in a [{radius}_tile:radius] radius are enfleshed. They become [living], lose [100_poison:poison] resistance, and gain [{hp_bonus}:living] max HP; the HP bonus is equal to 5 times the [damage] of this spell.\n"
                ...
                "If an enfleshed unit's current HP becomes equal to or below the max HP it gained from enfleshment, it dies instantly at the end of its turn; this is treated as killed by [poison] damage equal to the enfleshed amount.\n"
...
class MelodramaBuff(Thorns):

    def __init__(self, spell):
        self.spell = spell
        Thorns.__init__(self, spell.get_stat("damage"), Tags.Arcane)
        self.name = "Melodrama"
        self.color = Tags.Dark.color
    ...
    def on_advance(self):
        ...
        if not enemies:
            return
        random.shuffle(enemies)
        for enemy in enemies[:self.spell.get_stat("num_targets")]:
            dummy_hit = SimpleMeleeAttack(damage_type=Tags.Dark)
            dummy_hit.owner = enemy
            dummy_hit.caster = enemy
...
class MelodramaSpell(Spell):
    ...
    def on_init(self):
        self.name = "Melodrama"
        self.asset = ["MissingSynergies", "Icons", "melodrama"]
    ...
    def get_description(self):
        return ("Each turn, [{num_targets}:num_targets] random enemies in line of sight are chosen. You pretend to be targeted by and take [1_dark:dark] damage from a melee attack from each enemy, triggering all effects that are normally triggered by these events.\n"
                "For the duration, you also retaliate for [{damage}_arcane:arcane] damage when targeted in melee.\n"
                "Lasts [{duration}_turns:duration].").format(**self.fmt_dict())
...
class HolyBomberBuff(Buff):
    ...
    def get_tooltip(self):
        return "Dies at the end of turn. On death, causes Holy Hand Grenade to deal holy damage equal to max HP to enemies in a %i tile burst." % self.owner.source.get_stat("radius")
...
class HolyHandGrenadeSpell(Spell):
    ...
    def on_init(self):
        ...
        self.asset = ["MissingSynergies", "Icons", "holy_hand_grenade"]
        ...
        self.upgrades["radius"] = (1, 3)
        self.upgrades["minion_health"] = (8, 2)
        self.upgrades["blind"] = (1, 3, "Flashbang", "This spell also [blinds:blind] affected enemies for [1_turn:duration].")
        self.upgrades["thick"] = (1, 3, "Thick Shell", "The holy bomber gains bonus HP equal to 100% of its base HP.\nThis bonus HP does not contribute to the damage of its explosion.")
    ...
    def can_cast(self, x, y):
        if not Spell.can_cast(self, x, y):
            return False
        unit = self.caster.level.get_unit_at(x, y)
        return not unit or unit.source is self
    
    def get_total_hp(self):
        return self.get_stat("minion_health") + self.get_stat("minion_damage") + self.get_stat("damage")
    
    def fmt_dict(self):
        stats = Spell.fmt_dict(self)
        stats["total_hp"] = self.get_total_hp()
        return stats
    
    def get_description(self):
        return ("Summon a [holy] bomber, a flying stationary minion with [{total_hp}_HP:minion_health], which also benefits from bonuses to [minion_damage:minion_damage] and [damage].\n"
                "The bomber dies at the end of its turn, excluding the turn when it is summoned. On death, it causes this spell to deal [holy] damage equal to its max HP to enemies in a [{radius}_tile:radius] burst.\n"
                "Targeting an existing bomber with this spell immediately detonates it before summoning a new one.").format(**self.fmt_dict())

    def boom(self, hp, x, y):
        blind = self.get_stat("blind")
        for stage in Burst(self.caster.level, Point(x, y), self.get_stat("radius")):
            for p in stage:
                unit = self.caster.level.get_unit_at(p.x, p.y)
                if not unit or not are_hostile(unit, self.caster):
                    ...
                    unit.deal_damage(hp, Tags.Holy, self)
                    if blind:
                        unit.apply_buff(BlindBuff(), 1)
            yield
    ...
    def cast(self, x, y):
        ...
        unit = Unit()
        unit.name = "Holy Bomber"
        unit.tags = [Tags.Holy]
        unit.flying = True
        unit.stationary = True
        unit.max_hp = self.get_total_hp()
        unit.bonus_hp = 0
        if self.get_stat("thick"):
            unit.bonus_hp = unit.max_hp
            unit.max_hp *= 2
...
class MageGlasses(Upgrade):

    def on_init(self):
        self.name = "Mage Glasses"
        self.asset = ["MissingSynergies", "Icons", "mage_glasses"]
        self.tags = [Tags.Eye]
        self.level = 4
    
    def get_description(self):
        return ("All of your spells now have a base range of at least [8_tiles:range], excluding self-targeted and melee spells.\n"
                "This does not apply to minion spells inherited from you.").format(**self.fmt_dict())

    def fix_spell(self, spell):
        if spell.range < 2:
            return
        spell.range = max(spell.range, 8)
